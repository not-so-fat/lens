...
- [ ] (Claude Code) when a background lens `Agent`/`Task` was launched for this session (`subagent_type` names the lens runner) and no same-session `lens_run`/`lens_skip` exists since that launch, Stop **passes** (`skip_reason=review_in_flight`) so the parent is not force-continued while the child is fulfilling the obligation — not a third recorded exit and not an unpaid-obligation grace period; a crash backstop (wall-clock age since launch; see §7.6) restores blocking only when Stop can no longer treat the child as fulfilling
...
Canonical schema: `contracts/hook_check.schema.json`. Notable fields beyond the required core: `wrote_watched` (any watched activity in the session — M3 join key), `watched_writes` (unsatisfied remainder after Cursor `after_ts` filter), `armed` (session was armed by an explicit lens invocation — requires a `lens_run` or `lens_skip` regardless of writes, F3.5), `declined` (a `lens_skip` cleared the loop — the second exit, §7.2b), `session_ids`, `enforce`, `gate` (`none`|`time`|`session`|`declined`), `excluded_writes`, `skip_reason` (`declined` | `enforce_false` | `no_watch_match` | `excluded_only` | `review_in_flight`).

`review_in_flight` (Claude Code): **meaning** — a background lens `Agent`/`Task` (lens `subagent_type`, e.g. `lens:lens`) was launched and no same-session `lens_run`/`lens_skip` exists since that launch, so the child is fulfilling the obligation; Stop **passes** so the parent is not force-continued into a wait spin. This is not a recorded exit and not a grace period for unpaid work. **Crash backstop** — async Agent returns immediately, so Stop cannot observe liveness; if wall-clock age since launch exceeds 30 minutes with still no run/skip, Stop resumes blocking (treat the child as no longer fulfilling). Recorded exits (`lens_run` / `lens_skip`) still clear the gate immediately.
