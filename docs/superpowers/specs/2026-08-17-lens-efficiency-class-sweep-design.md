...
**Evidence:** 223 runs / 126 deliverables since 2026-08-01 (`~/.lens/lens_runs.jsonl`). Tail waste (≥5 rounds) clusters in `sibling-inconsistency` and `source-grounded`, dominated by fix-induced churn (instance fixed → sibling/cite drifts → caught one round late).
...
- No enumerated `class` vocabulary in schema (flexibility for one-offs and new patterns).
...
| Topic | Choice |
| --- | --- |
| Class field | Optional `class` on findings in the schema; runner **must** set it for repeatable patterns (one-offs omit it). Not schema-required on every finding. |
...
- `class` is a short stable label for the class under sweep (free string; kebab-case preferred). Examples of patterns that require it: stale cite, mirrored file, cross-reference, renumbered pointer — and any similar “fix one instance, siblings drift” shape.
...
1. Set `class` to a stable label for that class.
...
**A. Class verify** — For each prior finding that has `class`, re-sweep that class scope (not only the prior `target`), even when the reaction is `fixed` or `fixed-class`. Skip only if the worker disputed or the finding was escalated. New drift in the same class → same `check` + same `class` (new `target` / note as needed), not a fresh unlabeled instance.
...
- Set `class` to `fragile-line-cites`.
...
| File | Change |
| --- | --- |
| `contracts/lens_run.schema.json` | optional `class` |
| `docs/PRD.md` | §7.1 + new F-item under runner/findings (e.g. F2.4) with acceptance checkboxes |
...
- Finding schema accepts records with and without `class`; rejects unknown finding keys.
- Runners instruct: repeatable → set `class` + class-wide sweep; round ≥2 → ordered prelude A (class verify) → B (fix-regression) → C (Process); round 1 → cite-stability FIX (`fragile-line-cites`) when fragile line cites appear.
- Round semantics unchanged: terminal round still PASS or all-ESCALATE; log only on terminal.
- Claude/Cursor SHARED spans remain identical; Codex mirrors the same rules.

## Out of scope for this change

- Analytics jobs or log backfills.
- Enumerating allowed `class` values in schema.
